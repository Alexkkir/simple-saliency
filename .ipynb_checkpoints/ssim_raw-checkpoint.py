...
def ssim(x: torch.Tensor, y: torch.Tensor, mask, kernel_size: int = 11, kernel_sigma: float = 1.5,
    ...
    ssim_map = _ssim_per_channel(x=x, y=y, mask=mask, kernel=None, data_range=data_range, k1=k1, k2=k2)
...
def _ssim_per_channel(x: torch.Tensor, y: torch.Tensor, mask, kernel: torch.Tensor,
    ...
    c2 = k2 ** 2
    n_channels = x.size(1)
    mu_x = x #F.conv2d(x, weight=kernel, stride=1, padding=0, groups=n_channels)
    mu_y = y #F.conv2d(y, weight=kernel, stride=1, padding=0, groups=n_channels)

    mu_xx = mu_x ** 2
    mu_yy = mu_y ** 2
    mu_xy = mu_x * mu_y

    sigma_xx = x ** 2 - mu_xx #F.conv2d(x ** 2, weight=kernel, stride=1, padding=0, groups=n_channels) - mu_xx
    sigma_yy = y ** 2 - mu_yy #F.conv2d(y ** 2, weight=kernel, stride=1, padding=0, groups=n_channels) - mu_yy
    sigma_xy = x * y - mu_xy #F.conv2d(x * y, weight=kernel, stride=1, padding=0, groups=n_channels) - mu_xy
